...
public class Ranger : MonoBehaviour {
	...
	private void LevelUp()
	{
		...
		PlayerResource stamina = GetComponent<PlayerResource>();
		...
		if(level < 5)
			...
			health.maxHealth+=40;
			stamina.maxResource=25;}
		else if(level > 9 && level < 15){
			health.maxHealth+=80;
			stamina.maxResource=50;}
		else if(level >14 && level <20){
			health.maxHealth+=160;
			stamina.maxResource=75;}
		else if(level > 19 && level <25){
			health.maxHealth+=320;
			stamina.maxResource=100;}
		if(level == 25){
			health.maxHealth+=640;
			stamina.maxResource=100;}
		
		health.curHealth= health.maxHealth;
		stamina.curResource = stamina.maxResource;
	}
